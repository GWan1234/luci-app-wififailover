...
m = Map("wififailover", translate("WiFi Failover Configuration"),
    translate("Configure automatic WiFi network switching."))

if not fs.access("/sys/class/net/wlan0") then
    m.message = translate("WiFi 'wlan0' not found.")
end
...
function ping_host.validate(self, value)
    if value then
    if value:match("^%d+%.%d+%.%d+%.%d+$") or value:match("^[%w%.%-]+$") then return value end
    end
